...
impl EnvTypesTranscoder {
    ...
    // todo: allow registration of transcoders on construction of Transcode
    pub fn register_transcoder<T>(
...
impl From<Path> for PathKey {
    fn from(path: Path) -> Self {
        PathKey(path.segments().iter().map(ToString::to_string).collect())
    }
}
